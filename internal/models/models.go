...
type ProxyHost struct {
	...
	// v2.38.0: expectations_json — JSON array of post-apply checks
	// (HostExpectation). Run after every config sync; failures roll the live
	// config back and hold further syncs. See ExpectationList.
	Expectations string
	...
	// them.
	NodeLocal bool
	// v2.46.0: issue this host's certificate from Caddy's internal
	// (self-signed) CA instead of ACME. Intended for local-network services
	// where a publicly trusted certificate isn't wanted or possible. Only
...
const proxyHostBaseCols = `ph.id, ph.server_id, ph.domains, ph.forward_scheme, ph.forward_host, ph.forward_port,
    ...
    COALESCE(ph.additional_upstream_rules,''), COALESCE(ph.expectations_json,''),
...
func scanProxyHost(s interface {
	...
	dst := []any{
		...
		&p.AdditionalUpstreamRules, &p.Expectations,
...
func UpdateProxyHost(db *sql.DB, p *ProxyHost) error {
	...
	_, err := db.Exec(`
        UPDATE proxy_hosts SET domains=?, forward_scheme=?, forward_host=?, forward_port=?,
            ...
            additional_upstream_rules=?, expectations_json=?,
