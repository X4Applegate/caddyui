...
- **DNS-01 certificate issuance** — select Managed DNS on a proxy, redirection, or advanced route and CaddyUI reuses that credential profile for Let's Encrypt DNS-01. Supports Cloudflare, Porkbun, Namecheap, GoDaddy, DigitalOcean, Hetzner, and Amazon Route 53; the connected Caddy needs the matching `caddy-dns` plugin. Wildcards are supported, and public A-record creation can be disabled for internal/split-DNS services. *(Route 53 added in v2.23.0; DNS-01-only mode in v2.23.1)*
