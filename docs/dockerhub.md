...
description". Edit here first, then publish; the publisher strips this
...
**Open source under the [Apache License 2.0](https://github.com/X4Applegate/caddyui/blob/main/LICENSE).** Free for personal, commercial, and internal use, with no hosted-service restrictions.
...
- **OIDC / SSO login** *(v2.52)* — sign in through an external identity provider (Authelia, Authentik, Keycloak, Google, …) alongside local password + TOTP, with verified-email matching and opt-in auto-provisioning
- **Scheduled off-host backups** *(v2.52)* — automatic SQLite snapshots to a directory on an interval, keeping the newest N, plus a **Back up now** button; point it at a mounted volume to keep copies off the host
- **Per-host rate limiting** *(v2.51)* — cap a host at *N requests per M seconds, per client IP* (429 on excess), via the `caddy-ratelimit` module built into `Dockerfile.caddy`
- **Per-host HTTP Basic Auth** *(v2.52.6)* — bcrypt-hashed accounts enforced by Caddy as the last gate before the upstream
- **Block an IP from analytics** *(v2.50)* — one-click **Block on this host** or **Block everywhere (fleet-wide)** straight from a visitor's drill-down page
- **Analytics drill-downs** *(v2.47 – v2.48)* — click a visitor, a path or a status class to see exactly what happened: top paths, status mix, user-agents, and errors by exact status code + path
- **Internal CA (`tls internal`)** *(v2.46)* — serve internal or local-network hosts from Caddy's built-in self-signed CA instead of ACME, no special Caddy build required
- **Configurable verification resolver** *(v2.49)* — point deploy/readiness DNS checks at internal or split-horizon resolvers instead of Cloudflare DoH
...
- **Managed DNS** for Cloudflare, DigitalOcean, Hetzner, Porkbun, GoDaddy, Namecheap, Gandi, and Amazon Route 53 — auto-creates A records on save and reuses the credentials for DNS-01
...
- **Three-role RBAC** — admin / user / view, with per-user resource ownership and access groups whose members can manage each other's resources
...
[Apache License 2.0](https://github.com/X4Applegate/caddyui/blob/main/LICENSE) — CaddyUI is open source.
...
- **Patent grant** included, and contributions are welcome under the same license.
...
> CaddyUI was previously distributed under the CaddyUI Source Available License 1.0. It was relicensed to **Apache-2.0 in September 2026** — the old commercial-use and hosted-service restrictions no longer apply.
